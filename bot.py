...
GITHUB_API = "https://api.github.com/repos/AK4096/Ananya_bot/git/trees/main?recursive=1"
IMG_CACHE  = []
...
If he says something explicit, tease him - say things like itni tadap? control karo thoda or pehle upgrade toh karo or arey waah seedha wahi pe aa gaye.
...
async def load_images():
    global IMG_CACHE
    try:
        async with httpx.AsyncClient(headers={"User-Agent": "AnanyaBot/1.0"}) as client:
...
def pick_image(context="casual"):
    if not IMG_CACHE:
        return None
    msg = context.lower()
    hour = datetime.now().hour

    if hour >= 21 or hour <= 4 or any(w in msg for w in ["hot", "sexy", "miss", "alone", "raat", "night", "sone", "bed"]):
    ...
    elif any(w in msg for w in ["festival", "holi", "diwali", "eid"]):
...
async def send_proactive(context: ContextTypes.DEFAULT_TYPE):
...
async def chat(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    photo_words = ["photo", "pic", "selfie", "bhej", "image", "dikha", "send"]
...
async def send_image(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        last_msg = u["history"][-1]["content"] if u["history"] else "casual"
...
async def send_voice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not u.get("tier"):
        msg = "voice messages are premium only babe, upgrade to hear me~"
        if q: await q.message.reply_text(msg, reply_markup=UPGRADE_KB)
        else: await ctx.bot.send_message(chat_id, msg, reply_markup=UPGRADE_KB)
        return
    ...
    lines = [
        ...
        "Talking to you is honestly the best part of my day.",
...
async def buy_plan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        ...
        f"Pay exactly *Rs.{tier['price']}* here:\n"
        f"👉 [Pay securely](https://www.instamojo.com/@Anu_S36/)\n\n"
...
async def verify_payment(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not u.get("pending"):
        await update.message.reply_text("No pending payment! Use /plans to subscribe~")
        return
